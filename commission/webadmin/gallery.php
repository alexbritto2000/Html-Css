 ...
 if(!empty($_FILES["upload_file"]) && $_FILES["upload_file"]["error"]!=4) {
	...
	if ($_FILES['upload_file']['name'] != "") {
		...
		{ 
			...
			$parts=explode(".",$v);
			$v=$parts[0].".png";
			$originalpath = "../webupload/original/gallery/".$v;
			$thumbnailpath = "../webupload/thumb/gallery/".$v;
			move_uploaded_file($source,$originalpath);
			include_once('../include/imgresize.php');
			resize($originalpath, $thumbnailpath, 600, 350);
		}
		else
		{ 
...
if(!empty($_POST["submit_action"]) && (!empty($_POST["edit_action"]) || !empty($_POST["add_action"]))){   
	if(!empty($_POST["edit_action"])){
		$sql="UPDATE $table_name SET `gallery_image`='".$v."',`modified_date`='".$datetime."' WHERE `web_id`='".$db_cms->removeQuote($_POST["edit_action"])."'";
	}
	else{
		$sql="INSERT INTO $table_name(`gallery_image`,`created_date`) VALUES ('".$v."','".$datetime."')";
	}
                ...
                <div class="col-xs-12">
                    <div class="box">
                        <div class="box-header col-xs-11">
                            ...
                            if(!empty($_REQUEST["action"])){
                                ?>
                                <h3 class="box-title"><?php echo ucfirst($_REQUEST["action"]);?> Gallery Management</h3>
                        ...
                        <form role="form" class="form-horizontal" action="" method="post" enctype="multipart/form-data">
                            <div class="box-body">
                                <div class="form-group">
                                    <label class="col-sm-2 control-label">Gallery Image<span class="star">*</span>:</label>
                                    <div class="col-sm-6">
                                        <?php if($_REQUEST["action"]!="add"){  ?>
                                            <div>
                                                <img src="../include/image.php?width=500&amp;height=190&amp;cropratio=125:47&amp;image=<?php echo $sitepath;?>webupload/thumb/gallery/<?php echo $img_path;?>" alt="Image" style="width:500px;"/>
                                            </div>
                                            <br/>
                                        <?php  }  ?>
                                        <div class="form-group">
                                ...
                                if($res!=FALSE){
                                    ...
                                    foreach ($res as $row){
                                        ...
                                        <tr>
                                            ...
                                            <td align="center">
                                                <img src="../include/image.php?width=250&amp;height=95&amp;cropratio=125:47&amp;image=<?php echo $sitepath;?>webupload/thumb/gallery/<?php echo $row["gallery_image"];?>" alt="Image" style="width:150px;"/>
                                            </td>
