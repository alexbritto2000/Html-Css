...
$table_name="`".DB_PREFIX."_vision_objective`";
$current_page="objective_content.php";
...
if(!empty($_POST["submit_action"]) && (!empty($_POST["edit_action"]) || !empty($_POST["add_action"]))){
            if(!empty($_POST["title"]) && !empty($_POST["description"])){
				$content_page = "objective_content";
                ...
                $res=$db_cms->update_query($sql); print_r($res);  // <-  normal query function this
				//echo $sql; exit;
                if($res!=FALSE){
                    $_SESSION["cms_status"]="success";
                    if(!empty($_POST["edit_action"])){
    ...
    <div class="content-wrapper">
        ...
        <section class="content-header">
            ...
            <ol class="breadcrumb">
                ...
                <li class="active">Vision Content</li>
            </ol>
        </section>
                ...
                <div class="col-xs-12">
                    <div class="box">
                        <div class="box-header col-xs-11">
                            ...
                            if(!empty($_REQUEST["action"])){
                                ?>
                                <h3 class="box-title"><?php echo ucfirst($_REQUEST["action"]);?> Vision Content Management</h3>
                                <?php
                            }
                            else{
                                ?>
                                <div class="pull-right">
                                   <a class="btn display_none bg-maroon <?php echo ($is_add_enabled)?"":"disabled";?>" href="<?php echo ($is_add_enabled)?"?action=add":"javascript:void(0);";?>"><i class="fa  fa-plus"></i> &nbsp;Add</a>
                                </div>
                                <?php
                            }
                            ?>
